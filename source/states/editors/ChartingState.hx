...
import hybridengine.backend.Event;
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		["Zoom Camera", "Value 1: Zoom Value\nValue 2: Time it takes to change fully in seconds."]
	];
	...
	var eventLists:Array<Event> = [];
	var eventUI:Array<FlxObject> = [];
	var eventVals:Array<String> = [];
	...
	var bg:FlxSprite;
	var theme:ChartingTheme = VSLICE;
	...
	{	
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/states/charteditor/'))
		...
		add(waveformSprite);
		callOnHScript("onLoad",["waveformSprite",waveformSprite]);
		...
		updateVortexColor();
		add(vortexIndicator);
		callOnHScript("onLoad",["vortexIndicator",vortexIndicator]);
		add(strumLineNotes);
		callOnHScript("onLoad",["strumLineNotes",strumLineNotes]);

		add(behindRenderedNotes);
		callOnHScript("onLoad",["behindRenderedNotes",behindRenderedNotes]);
		add(curRenderedNotes);
		callOnHScript("onLoad",["curRenderedNotes",curRenderedNotes]);
		add(movingNotes);
		...
		callOnHScript("onLoad",["eventLockOverlay",eventLockOverlay]);
		...
		add(timeLine);
		callOnHScript("onLoad",["timeLine",timeLine]);
		...
		for (i in 0...Std.int(GRID_PLAYERS * GRID_COLUMNS_PER_PLAYER))
		{
			...
			callOnHScript("onLoad",["note",note]);
		}
		...
		{
			eventIcon = new FlxSprite(0, iconY).loadGraphic(Paths.image('editors/eventIcons/defaultEventIcon'));
			...
			callOnHScript("onLoad",["eventIcon",eventIcon]);
		}
		...
		callOnHScript("onLoad",["mustHitIndicator",mustHitIndicator]);
		...
		for (i in 0...GRID_PLAYERS)
		{
			...
			FlxTween.tween(icon, {alpha: 0.6,y:50}, 1.5, {ease: FlxEase.circOut});

			callOnHScript("onLoad",["icon",icon]);
		}
		...
		add(selectionBox);
		callOnHScript("onLoad",["selectionBox",selectionBox]);
		...
		mainBox = new PsychUIBox(mainBoxPosition.x, mainBoxPosition.y, 300, 330, ['Charting', 'Data', 'Note', 'Section', 'Song']);
		...
		eventBox = new PsychUIBox(20, 40, 300, 290, ['Hybrid Event (BETA)', 'Psych Event']);
		eventBox.selectedName = 'Hybrid Event (BETA)';
		eventBox.scrollFactor.set();
		eventBox.cameras = [camUI];
		add(eventBox);
		callOnHScript("onLoad",["eventBox",eventBox]);
		...
		add(autoSaveIcon);
		callOnHScript("onLoad",["autoSaveIcon",autoSaveIcon]);
		...
		upperBox = new PsychUIBox(0, 0, 300, 600, ['File', 'Edit', "Test", 'View', "Editors", "Help"],false,26);
		...
		add(upperBox);
		callOnHScript("onLoad",["upperBox",upperBox]);
		...
		outputTxt.alpha = 0;
		add(outputTxt);
		callOnHScript("onLoad",["outputTxt",outputTxt]);
		...
		add(tipText);
		callOnHScript("onLoad",["tipText",tipText]);
		...
		tipBg.alpha = 0.6;
		add(tipBg);
		callOnHScript("onLoad",["tipBg",tipBg]);
		...
		add(fullTipText);
		callOnHScript("onLoad",["fullTipText",fullTipText]);
		super.create();
	...
	override function update(elapsed:Float)
	{
		...
		if(PsychUIInputText.focusOn == null && lastFocus == null)
		{
			...
			if(doCut || FlxG.keys.justPressed.DELETE || FlxG.keys.justPressed.BACKSPACE || (isMovingNotes && (FlxG.mouse.justPressedRight || FlxG.keys.justPressed.ESCAPE))) // Delete button
			{
				if(selectedNotes.length > 0)
				{
					FlxG.sound.play(Paths.sound('chartingSounds/noteErase'), 0.6);
		...
		if(FlxG.mouse.justPressed && (FlxG.mouse.overlaps(mainBox.bg) || FlxG.mouse.overlaps(infoBox.bg) || FlxG.mouse.overlaps(eventBox.bg)))
				...
				else if(FlxG.mouse.x >= gridBg.x && FlxG.mouse.x < gridBg.x + gridBg.width)
				{
					...
					else if(!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height) // Add note
					{
						...
						else if(!lockedEvents)
						{
							...
							var icon:String = "defaultEventIcon";
							if (eventBox.selectedName == "Hybrid Event (BETA)") {
								// value = [strumTime, [[eventLists[Std.int(Math.max(hybridDropDown.selectedIndex, 0))].eventName, "", "", ""]], [
								// 	icon,
								// 	eventVals
								// ]];
							...
							var eventAdded:EventMetaNote = createEvent(value);
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			if(metronomeStepper.value > 0 && lastBeatHit != curBeat) {
				...
				if (curBeat % 4 == 0) {
					FlxG.sound.play(Paths.sound('chartingSounds/metronome1'), metronomeStepper.value);
				} else {
					FlxG.sound.play(Paths.sound('chartingSounds/metronome2'), metronomeStepper.value);
				}
			}
	...
	function loadMusic(?killAudio:Bool = false)
	{
		...
		try
		{
			...
			FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song,true,isErect), 0);
		...
		if (PlayState.SONG.needsVoices)
		{
			try
			{
				...
				var playerVocals:Sound = Paths.voices(PlayState.SONG.song, (characterData.vocalsP1 == null || characterData.vocalsP1.length < 1) ? 'Player' : characterData.vocalsP1,true,isErect);
				...
				var oppVocals:Sound = Paths.voices(PlayState.SONG.song, (characterData.vocalsP2 == null || characterData.vocalsP2.length < 1) ? 'Opponent' : characterData.vocalsP2,true,isErect);
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		callOnHScript("onLoadSection",[sec]);
	}
	...
	function updateHeads(ignoreCheck:Bool = false):Void
	{
		...
		for (i in 0...GRID_PLAYERS)
		{
			...
			icon.alpha = 0;
			FlxTween.tween(icon, {alpha: 0.6,y:50}, 1, {ease: FlxEase.circOut});
		}
		...
		callOnHScript("onUpdateHeads",[ignoreCheck]);
	}
	...
	function addChartingTab()
	{
		...
		objY += 50;
		hitsoundPlayerStepper = new PsychUINumericStepper(objX, objY, 0.2, 0.1, 0, 1, 1);
		hitsoundOpponentStepper = new PsychUINumericStepper(objX + 100, objY, 0.2, 0.1, 0, 1, 1);
	...
	var hybridDropDown:PsychUIDropDownMenu;
	...
	function addPsychEventTab()
	{
		var tab_group = eventBox.getTab('Psych Event').menu;
		var objX = 10;
		var objY = 25;
		...
		value3InputText = new PsychUIInputText(objX + 150, objY + 50, 120, '', 8);
	...
	public function loadHybridEvents() {

		var ignoreNewEvent:Map<String,Bool> = [];

		ignoreNewEvent.set("Set Camera Target", true);
		ignoreNewEvent.set("Run Haxe Code", true);

		var eventHE:Event = new Event("Set Camera Target");
		...
		for (directory in Mods.directoriesWithFile(Paths.getSharedPath(), "scripts/events/"))
		{
			...
			{
				if (ignoreNewEvent.exists(file)) continue;
				var eventHE:Event = new Event(file);
				eventLists.push(eventHE);
			}
		}
	}

	function addHybridEventTab()
	{
		var tab_group = eventBox.getTab('Hybrid Event (BETA)').menu;
		var objX = 10;
		var objY = 25;

		loadHybridEvents();

		var eventStringList:Array<String> = [];

		for (event in eventLists)
		{
			eventStringList.push(event.eventName);
		}
		...
		tab_group.add(hybridDropDown); //lowest priority to display properly
	}

	public function reloadEventBox(tab_group:Dynamic, index:Int) {
		var targetEvent:Event = eventLists[index];

		var objX = 10;
		var objY = 85;

		for (ui in eventUI) { tab_group.remove(ui); }
		eventUI = [];
		...
		for (eventIndex in 0...targetEvent.eventData.length)
		{
			...
			var type:EventValueTypes = eventData[1];

			if (type == EventValueTypes.STRING) {
				...
				eventUI.push(inputText);

				eventVals[eventIndex] = eventData[2];

			} else if (type == EventValueTypes.FLOAT) {
				var nameText = new FlxText(objX, objY - 15, 80, eventData[0]+":");
				...
				eventUI.push(nameText);
				eventUI.push(stepper);

				eventVals[eventIndex] = Std.string(eventData[2]);

			} else if (type == EventValueTypes.GAME_CHARACTERS) {
				var nameText = new FlxText(objX, objY - 15, 80, eventData[0]+":");
				...
				eventUI.push(nameText);
				eventUI.push(characterDropDown);

				characterDropDown.selectedLabel = eventData[2];
				eventVals[eventIndex] = eventData[2];

			} else if (type == EventValueTypes.DROP_DOWN) {
				var nameText = new FlxText(objX, objY - 15, 80, eventData[0]+":");
				...
				eventUI.push(nameText);
				eventUI.push(dropDown);

				dropDown.selectedLabel = eventData[2];
				eventVals[eventIndex] = eventData[2];

			} else if (type == EventValueTypes.BOOLEAN) {
				var nameText = new FlxText(objX, objY - 15, 80, eventData[0]+":");
		...
		eventUI.reverse();
		for (ui in eventUI) { tab_group.add(ui); }
	}
	...
	function addHelpTab() {
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Keybinds', function()
		{
			var vis:Bool = !fullTipText.visible;
			tipBg.visible = tipBg.active = fullTipText.visible = fullTipText.active = vis;
		}, btnWid);
	...
	function addEditorsTab() {
		var tab = upperBox.getTab('Editors');
		var tab_group = tab.menu;
		var btnX = tab.x - upperBox.x;
		var btnY = 1;
		var btnWid = Std.int(tab.width);

		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Character', function()
		{

			var func:Void->Void = function()
				{
					MusicBeatState.switchState(new CharacterEditorState(PlayState.SONG.player2,false,true));
				}
	
			if(!ignoreProgressCheckBox.checked) openSubState(new Prompt('Are you sure you want to go to the Character Editor?', func));
			else func();
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Stage', function()
		{

			var func:Void->Void = function()
				{
					MusicBeatState.switchState(new StageEditorState(PlayState.SONG.stage,null,true));
				}
	
			if(!ignoreProgressCheckBox.checked) openSubState(new Prompt('Are you sure you want to go to the Stage Editor?', func));
			else func();
